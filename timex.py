...
class NltkTimex(object):
    ...
    def ground(self, tagged_text, base_date):
        ...
        for timex in timex_found:
            ...
            if re.search(numbers, timex, re.IGNORECASE):
                ...
                value = split_timex[0]
                unit = split_timex[1]
                num_list = map(lambda s:self.hashnum(s),re.findall(numbers + '+', \
                                              value, re.IGNORECASE))
    ...
    '''
        Timex is provided by nltks itself. I have used only required regex out of all. 
    '''
    def demo(self, text):
        print(self.tag(text))

if __name__ == '__main__':
    text = input("Please enter the query: \n") 
    NltkTimex().demo(text)
